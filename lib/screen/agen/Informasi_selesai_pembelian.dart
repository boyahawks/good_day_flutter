...
class InformasiSelesaiPembelian extends StatefulWidget {
  @override
  State<InformasiSelesaiPembelian> createState() =>
      _InformasiSelesaiPembelianinState();

  var kedai, pointBranding, pointHanger, statusKedai, pointPembelian;
  InformasiSelesaiPembelian(
    this.kedai,
    this.pointBranding,
    this.pointHanger,
    this.statusKedai,
...
class _InformasiSelesaiPembelianinState
    extends State<InformasiSelesaiPembelian> {
  // final GlobalKey _scaffoldKey = new GlobalKey();

  _InformasiSelesaiPembelianinState createState() =>
      _InformasiSelesaiPembelianinState();
  ...
  aksiKirimLaporan() async {
    ...
    try {
      var formData = dio.FormData.fromMap({
        ...
        'point_pembelian': point_pembelian,
      });
      print(formData);
      var response = await dio.Dio().post(
      ...
      setState(() {
        if (status == true) {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => DashboardAgen()),
          );
        } else {
          WidgetAlert.showAlertUmum(context, message);
          print("$message");
                ...
                children: [
                  ...
                  Container(
                      ...
                      child: ListView.builder(
                          ...
                          itemCount: controller.listProdukTerpilih.length,
                          itemBuilder: (context, index) {
                            return Row(children: [
                              Expanded(
                                flex: 80,
                                child: Text(
                                  "${controller.listProdukTerpilih[index]['nama_produk']}",
                                  textAlign: TextAlign.left,
                                ),
                              ),
                              Expanded(
                                flex: 20,
                                child: Text(
                                  "${controller.listProdukTerpilih[index]['jumlahPilih']}",
                                  textAlign: TextAlign.right,
                                ),
                              )
                            ]);
